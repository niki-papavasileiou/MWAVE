...
def display_ellipse(user):
    ...
    values_map = df[user].to_numpy()
    print(max(values_map))

    dangerous_points = np.column_stack((longitudes[values_map > threshold], latitudes[values_map > threshold]))
    
    if dangerous_points.shape[0] >= 3:
        dbscan = DBSCAN(eps=2, min_samples=3).fit(dangerous_points)
        ...
        if -1 in unique_labels and len(unique_labels) == 1:  
            ...
            cs = ax.scatter(longitudes, latitudes, c=values_map, vmin=vmin, vmax=vmax, cmap='jet', transform=ccrs.PlateCarree())
...
def display(user):
    ...
    cs = ax.scatter(lons, lats, c=z, vmin=vmin, vmax=vmax, cmap='jet', transform=ccrs.PlateCarree())
